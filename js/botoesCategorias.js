...
function mudarFocoPorTeclado(evento) {
  ...
  if (evento.key === "ArrowRight") {
    if (botaoAtual === tablist.lastElementChild) {
      const primeiroBotao = tablist.firstElementChild;
      primeiroBotao.focus();
      primeiroBotao.scrollIntoView({ behavior: "smooth", block: "nearest" });
    } else {
      const proximoElemento = botaoAtual.nextElementSibling;
      if (proximoElemento) {
        proximoElemento.focus();
        proximoElemento.scrollIntoView({ behavior: "smooth", block: "nearest" });
      }
    }
  } else if (evento.key === "ArrowLeft") {
    if (botaoAtual === tablist.firstElementChild) {
      const ultimoBotao = tablist.lastElementChild;
      ultimoBotao.focus();
      ultimoBotao.scrollIntoView({ behavior: "smooth", block: "nearest" });
    } else {
      const elementoAnterior = botaoAtual.previousElementSibling;
      if (elementoAnterior) {
        elementoAnterior.focus();
        elementoAnterior.scrollIntoView({ behavior: "smooth", block: "nearest" });
      }
    }
